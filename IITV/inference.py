...
import evaluation
from test_model import Improved_ITV
...
def parse_args():
    ...
    parser.add_argument('--logger_name', default='checkpoints', help='Path to save the model and Tensorboard log.')
...
def main():
    ...
    for query_set in query_sets:
        ...
        #UL concept sim
        pred_result_file = os.path.join(output_dir_tmp, 'id.sent.embedding_search_score.txt')
        embedding_matrix = embedding_matrix_all[query_idx,:]

        embedding_matrix_inds = np.argsort(embedding_matrix, axis=1)
        with open(pred_result_file, 'w') as fout:
            for index in range(embedding_matrix_inds.shape[0]):
                ind = embedding_matrix_inds[index][::-1]
                fout.write(query_ids[index] + ' ' + ' '.join(
                    [vis_ids[i] + ' %s' % embedding_matrix[index][i] for i in ind]) + '\n')
        print("embedding search time: %.3f s" % (time.time() - start))


        #UL concept sim
        pred_result_file = os.path.join(output_dir_tmp, 'id.sent.concept_search_score.txt')

        UL_concept_decoded_matrix= UL_concept_decoded_combined_matrix_all[query_idx,:]
        UL_concept_decoded_matrix_inds = np.argsort(UL_concept_decoded_matrix, axis=1)
        with open(pred_result_file, 'w') as fout:
            for index in range(UL_concept_decoded_matrix_inds.shape[0]):
                ind = UL_concept_decoded_matrix_inds[index][::-1]
                fout.write(query_ids[index] + ' ' + ' '.join(
                    [vis_ids[i] + ' %s' % UL_concept_decoded_matrix[index][i] for i in ind]) + '\n')
        print("concept search time: %.3f s" % (time.time() - start))
